...
const trackedChannelEnvKeys = ['OM_DURABLE_PLATFORM_CHANNEL', 'PLATFORM_SYNC_CHANNEL']
...
export function resolveChannel({ repoRoot, explicitChannel, env = process.env }) {
  ...
  if (!channel) {
    ...
    throw new Error(
      `Could not resolve platform channel from ${branchLabel}. Pass --channel develop|latest or set OM_DURABLE_PLATFORM_CHANNEL.`
    )
  }
...
function createExpectedTempRepo(repoRoot, expectedManifestMap) {
  const tempRepoRoot = mkdtempSync(path.join(os.tmpdir(), 'om-durable-platform-sync-'))
...
function buildHelpText() {
  return [
    ...
    '  --package <name...>     Limit sync to selected workspace package names, for example sandbox @fullstackhouse/open-mercato-durable-work',
