...
- **In the server process** — call `startInProcessWorker()` from the app's bootstrap (`instrumentation.ts` on Next). The call is the opt-in; there is no flag to also set. A start that fails is retried with backoff (5s doubling to 60s) rather than cached: the host calls this once at boot, so a failed start would otherwise last the life of the process, and readiness probes cannot see it. No second container, no second Deployment, and nothing for a host to forget — which matters, because forgetting is silently fatal: runs are created, leased by nobody, and parked by the reconciler much later. A deploy stopping the server mid-slice is the case the mechanism is built for; the lease expires and another replica resumes from the committed cursor.
